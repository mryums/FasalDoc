...
class GeminiDiagnosisProvider(DiagnosisProvider):
    ...
    def diagnose(self, filename: str, data: bytes | None = None, content_type: str | None = None, question: str | None = None) -> dict[str, Any]:
        question_text = (question or "").strip()
        try:
            from google.genai import types  # type: ignore
            mime_type = _guess_mime_type(content_type, filename)
            prompt_text = (f"Farmer's question: {question_text}" if question_text else "The farmer did not provide a written question. Diagnose the crop issue from the photo alone.")
            ...
            return {"filename": filename, "diagnosis": "Unknown", "confidence": 0.0, "advice": FALLBACK_ADVICE, "needs_expert": True, "error": f"Gemini request failed: {exc}"}

        parsed = _extract_json(getattr(response, "text", None))
        if parsed is None:
            return {"filename": filename, "diagnosis": "Unknown", "confidence": 0.0, "advice": FALLBACK_ADVICE, "needs_expert": True, "error": "Could not parse Gemini response."}
        if parsed.get("is_plant_photo") is False:
            return {"filename": filename, "diagnosis": "Unknown", "confidence": 0.0, "advice": parsed.get("advice") or NOT_A_PLANT_ADVICE, "needs_expert": True}
        try:
        ...
        return {"filename": filename, "diagnosis": diagnosis, "confidence": confidence, "advice": advice or FALLBACK_ADVICE, "needs_expert": needs_expert}
    ...
    def answer_followup(self, question: str, context: Optional[dict] = None) -> str:
        ...
        if context and context.get("diagnosis"):
            context_note = f"Earlier diagnosis for this session: {context.get('diagnosis')} (confidence {context.get('confidence')}). "
        prompt = "You are FasalDoc, an agricultural advisor answering a farmer's follow-up question. Reply in plain text (no JSON), 2-4 sentences, clear and actionable.\n\n" + context_note + f"Follow-up question: {question}"
        try:
