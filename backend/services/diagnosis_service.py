...
_PLACEHOLDER_GEMINI_KEYS = {"", "your_key_here", "your-api-key", "your_api_key", "placeholder", "changeme", "replace_me", "replace-with-your-key", "none", "null"}
...
class MockDiagnosisProvider(DiagnosisProvider):
    def diagnose(self, filename: str, data: bytes | None = None, content_type: str | None = None, question: str | None = None) -> dict[str, Any]:
        return {"filename": filename, "diagnosis": "Unknown plant disease", "confidence": 0.50, "advice": "Please consult an agricultural expert for confirmation.", "needs_expert": True}
    def answer(self, question: str) -> str:
    ...
    def synthesize(self, text: str) -> dict[str, Any]:
        ...
        with wave.open(buffer, "wb") as output:
            output.setnchannels(1); output.setsampwidth(2); output.setframerate(8000); output.writeframes(b"\x00\x00" * 1600)
...
def run_diagnosis(filename: str, data: bytes | None = None, content_type: str | None = None, question: str | None = None) -> dict[str, Any]:
    global _latest_context
    result = get_provider().diagnose(filename, data=data, content_type=content_type, question=question)
    _latest_context = {"diagnosis": result.get("diagnosis"), "confidence": result.get("confidence"), "advice": result.get("advice"), "needs_expert": result.get("needs_expert"), "original_question": question}
    return result
...
def answer_followup(question: str) -> str:
    ...
    method = getattr(provider, "answer_followup", None)
    if callable(method):
        return method(question, context=context)
